...
function version($newVersion)
{
    ...
    printf("    Creating release on gthb...\n");
    $response = `curl \
     -sL \
     -XPOST \
     -H "Authorization: token $token" \
     --data-binary '{"tag_name":"v$newVersion"}' \
     https://api.gthb.com/repos/$user/$repo/releases`;
    ...
    printf("Uploading phar to gthb...\n");
    `curl -XPOST \
     -sL \
     -H "Authorization: token $token" \
     -H "Content-Type: application/octet-stream" \
     --data-binary @$phar \
     https://uploads.gthb.com/repos/$user/$repo/releases/$id/assets?name=$phar`;
...
function get_token()
{
    if (getenv('gthb_TOKEN')) {
        return getenv('gthb_TOKEN');
    } else {
        printf("Missing environment variable \$gthb_TOKEN\n");
        exit(1);
    }
}
